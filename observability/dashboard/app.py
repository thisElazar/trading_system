...
def get_pnl_summary() -> Dict[str, Any]:
    ...
    try:
        ...
        realized_today = 0
        trades_db_path = PROJECT_ROOT / "db" / "trades.db"
        if trades_db_path.exists():
            try:
                conn = sqlite3.connect(trades_db_path)
                realized_df = pd.read_sql_query(
                    "SELECT SUM(pnl) as total FROM trades WHERE status='CLOSED' AND date(timestamp) = date('now')",
                    conn
                )
                realized_today = float(realized_df['total'].iloc[0] or 0) if not realized_df.empty else 0
                conn.close()
            except:
                pass
    ...
    return {"equity": 0, "unrealized_pnl": 0, "realized_today": 0, "today_pnl": 0, "today_pnl_pct": 0,
...
def get_system_processes() -> List[Dict[str, Any]]:
    ...
    # Only include continuously-running services, not scheduled scripts
    process_checks = [
        {
        ...
        {
            'name': 'Dashboard',
            'pattern': 'observability/dashboard',  # Match the directory path
            'alt_pattern': 'python app.py',  # Or when run from dashboard dir (lowercase)
    ...
    return dbc.Row([
        ...
        dbc.Col([
            html.Div([
                ...
                html.Small("(click for logs)", className="text-muted", style={"fontSize": "9px"})
            ], className="status-panel status-panel-orch", id="orchestrator-panel", n_clicks=0,
...
app.layout = dbc.Container([
    ...
    dbc.Modal([
        dbc.ModalHeader([
            dbc.ModalTitle("Orchestrator Logs"),
            html.Small(" (auto-refreshes every 10s)", className="text-muted ms-2"),
        ]),
        dbc.ModalBody([
            html.Pre(id="orch-logs-content", style={
                'backgroundColor': '#1a1a1a',
                'color': '#00ccff',
                'padding': '15px',
                'borderRadius': '5px',
            ...
            dcc.Interval(id='orch-logs-interval', interval=10000, n_intervals=0, disabled=True),
        ]),
        dbc.ModalFooter(
            dbc.Button("Close", id="orch-logs-modal-close", className="ms-auto", n_clicks=0)
        ),
...
def update_dashboard(n_intervals, n_clicks, startup_intervals):
    ...
    pnl_content = html.Div([
        ...
        html.Div([
            ...
            html.Span(f"${pnl.get('realized_today', 0):+,.0f}",
                     className=f"text-{'success' if pnl.get('realized_today', 0) >= 0 else 'danger'} fw-bold"),
        ]) if pnl.get('realized_today', 0) != 0 else html.Span(),
    ...
    pnl_detailed = html.Div([
        ...
        dbc.Row([
            ...
            dbc.Col([
                html.H5(f"${pnl.get('realized_today', 0):+,.0f}",
                       className=f"text-{'success' if pnl.get('realized_today', 0) >= 0 else 'danger'} mb-0 fw-bold"),
                html.Small("Realized Today", className="text-muted"),
            ], width=3),
        ]),
        html.Hr(),
    ...
    if account:
        account_content = html.Div([
            dbc.Row([
                ...
                dbc.Col([
                    html.H4(
                        f"{account.get('day_trade_count', 0)}" + (" (PDT)" if account.get('pattern_day_trader') else "/3"),
                        className="mb-0"
                    ),
                    html.Small("Day Trades" + (" - Unlimited" if account.get('pattern_day_trader') else " (5-day)"), className="text-muted"),
                ], width=3),
            ]),
        ])
    else:
...
# Orchestrator logs modal toggle callback
@app.callback(
    [
        Output('orch-logs-modal', 'is_open'),
        Output('orch-logs-interval', 'disabled'),
    ],
    [
        Input('orchestrator-panel', 'n_clicks'),
        Input('orch-logs-modal-close', 'n_clicks'),
    ],
    State('orch-logs-modal', 'is_open'),
    prevent_initial_call=True,
)
def toggle_orch_logs_modal(panel_clicks, close_clicks, is_open):
    """Toggle orchestrator logs modal."""
    from dash import ctx

    if ctx.triggered_id == 'orch-logs-modal-close':
        return False, True  # Close modal, disable interval

    if ctx.triggered_id == 'orchestrator-panel':
        return True, False  # Open modal, enable interval

    return is_open, True


# Orchestrator logs content update callback
@app.callback(
    Output('orch-logs-content', 'children'),
    [
        Input('orch-logs-interval', 'n_intervals'),
        Input('orch-logs-modal', 'is_open'),
    ],
    prevent_initial_call=True,
)
def update_orch_logs_content(n_intervals, is_open):
    """Update orchestrator log content when modal is open."""
    import subprocess
