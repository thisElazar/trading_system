...
Usage:
    ...
    python scripts/daily_data_refresh.py --full    # Refresh all cached symbols
...
def refresh_daily_data(symbols: list = None, force: bool = False) -> dict:
    """
    ...
    for i, symbol in enumerate(symbols):
        try:
            ...
            if df is not None and not df.empty:
                results["success"] += 1
                if (i + 1) % 50 == 0:
                    logger.info(f"Progress: {i + 1}/{len(symbols)} symbols refreshed")
            else:
                results["skipped"] += 1
        except Exception as e:
            results["failed"] += 1
...
def main():
    ...
    if args.test:
        ...
        logger.info(f"FULL MODE: Refreshing all {len(symbols)} cached symbols")
    else:
        symbols = get_sp500_symbols()
        logger.info(f"STANDARD MODE: Refreshing {len(symbols)} SP500 symbols")

    # Refresh daily bars
    results = refresh_daily_data(symbols, force=args.force)
